'''
    ...
    Description : Generates a AWG pulse sequence to make the ions dance to an audio file
...
def alternatingPulse() :
        
    innitPulseList()
    
    ONfchan = COOLING_FCHAN
    OFFfchan = NULL_CHAN
        
    for i in range(10) :
        addRFShapedPulse('Ions blink', 1000000, 0, 0, MWOUT, ONfchan)
        addRFShapedPulse('Ions blink', 1000000, 0, 0, MWOUT, OFFfchan)

    addParam("steps", 1) 
    addParam("runs", runs)
    addParam("comment", "statedetone")
    mathparams = {"name" : "statedetone", "det" : 0, "step" : 0, "state" : 'state', "freq" : 0, "pitime" : 0, "amp" : 0, "steps" : 1, "runs" : runs, 'err' : ERR_PARAMS}
